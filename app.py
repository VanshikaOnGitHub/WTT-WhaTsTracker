...
st.sidebar.title('WTT-WhaTsTracker')
uploaded_file = st.sidebar.file_uploader('Select file')
if uploaded_file is not None:
    ...
    group_name = st.sidebar.text_input('Group Name (if groupchat)')
    ...
    if st.sidebar.button('Display Analysis'):
        ...
        if selected_user == 'Overall':
            ...
            with col1:
                st.subheader('Most Busy Users', help='Users who message the most')
                fig.add_trace(
                    go.Bar(x=x['user'], y=x['message']),
                    row=1, col=1)
                fig.update_layout(
        ...
        st.subheader('Day Activity Heatmap',
                    help='Dark colors show low activity and brighter ones shows more activities. This might help you understand, when to have conversations in your chat')
        ...
        # WordCloud
        st.title('Most Spoken Words and Phrases',
                 help='This section contains analysis of words and phrases used in the chat')
        ...
        if keyword is not None and keyword != '':
            ...
            fig = px.line(df_keyword.groupby(['date', 'user']).sum().reset_index(), x="date", y="keyword_count",
            ...
            fig.update_layout(
                ...
                xaxis_title="Time",
                yaxis_title="Number of times {} was used (in a day)".format(keyword),
                font=dict(
                    family="Courier New, monospace",
                    size=14,
                    color="turquoise"
        ...
        st.header('Sentiment analysis', help='Sentiment analysis of all messages in chat')
        ...
        fig = px.pie(df_sentiment, values='frequency', names='sentiment', color='sentiment',
        ...
        fig = px.line(df_sentiment_timeline, x="date", y="positive", color='user', markers=True)
        fig.update_layout(
            title={
        ...
        fig = px.line(df_sentiment_timeline, x="date", y="neutral", color='user', markers=True)
        fig.update_layout(
            title={
        ...
        fig = px.line(df_sentiment_timeline, x="date", y="negative", color='user', markers=True)
        fig.update_layout(
            title={
        ...
        #emoji analysis
        st.subheader('User wise Emoji Count')
        fig = px.line(df_sender_emoji_count, x="date", y="n_emoji", color='user', markers=True)
        fig.update_layout(
            title={
                'text': 'Timeline comparison of user\'s emoji count',
                'x': 0.5,
                'y': 0.95,
                'xanchor': 'center',
                'yanchor': 'top'
